...
<?php if($_SESSION['msg']!=""){ ?>
  ...
  //print_r($rshasil);
  ?>
  <h2>Daftar <span class="label label-primary">Rute Destinasi</span> <?php echo $dbu->lookup("nama","destinasi_bahasa","id_reff ='".$dbu->lookup("id_reff","destinasi","id='".$p_id."'")."'"); ?></h2>
  <br/> 
    ...
    <tbody>
      ...
      ?>
        <tr <?php echo($nors % 2 == 1)?'class="odd"':'class="even"'; ?>>
          ...
          <td style="text-align:center">
            <?php echo $dbu->lookup("nama","kota","id='".$rshasil[id_from]."'"); ?>
          </td>
          <td style="text-align:center">
            <?php echo $dbu->lookup("nama","kota","id='".$rshasil[id_to]."'"); ?>
          </td>
          <td>
