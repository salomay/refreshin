...
<?php if($_SESSION['msg']!=""){ ?>
  ...
  //print_r($app[me]);
  ?>
  <h2>Merubah Data <span class="label label-primary">Bahasa</span></h2>
  <br/> 
...
<div>
	...
	<select name="p_negara" id="p_negara" >
		...
		while($frsp=$dbu->fetch($rsp)){
		 ?>
			<option value="<?php echo $frsp[id]; ?>" <?php echo ($form[id_negara]==$frsp[id])?"selected":""; ?>><?php echo $frsp[nama]; ?></option>
		<?php } ?>
	</select>
	</div>
</div>
<div>
	<div>id *</div>
	<div>
	<input name="p_idbhs" type="text" id="p_idbhs" value="<?php echo $form[id]; ?>">
	</div>
</div>
<div>
	<div>bahasa *</div>
	<div>
	<input name="p_bahasa" type="text" id="p_bahasa" value="<?php echo $form[bahasa]; ?>">
	</div>
</div>
<br/>
