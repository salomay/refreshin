...
<?php if($_SESSION['msg']!=""){ ?>
  ...
  //print_r($app[me]);
  ?>
  <h2>Membuat <span class="label label-primary"> Destinasi Bahasa</span></h2>
  <br/> 
...
<div>
	...
	$sql= "SELECT distinct(id_bahasa) from ".$app[table][destinasi_bahasa]." where id_reff='".$p_id."' order by id";
	//echo $sql;
	$dbu->query($sql,$rsp,$nrp);
	...
	<select name="p_bahasa" id="p_bahasa" >
		<?php
		 ...
		 ?>
			<option value="<?php echo $k; ?>"><?php echo $dbu->lookup("bahasa","bahasa","id='".$k."'"); ?></option>
		<?php } ?>
	</select>
	</div>
</div>
...
<div>
	<div>nama spot *</div>
	<div>
	<input value="<?php echo $form[spot]; ?>" name="p_spot" type="text" id="p_spot" placeholder="spot surabaya ...">
	</div>
</div>
<br/>
<div>
