...
if ($act == "browse"){
	...
	$sql = "SELECT a.*, b.username FROM ".$app['table']["destinasi_biaya"]." a LEFT JOIN ".$app['table']["pengguna"]." b ON(a.id_user=b.id) $where $sort $limit";
	//echo $sql;exit;
...
if ($act == "delete"){
	...
	if (!empty($_REQUEST[item])){
		$sql = "DELETE FROM ".$app['table']["destinasi_biaya"]." WHERE `id` IN (".$admlib->item_delete($_REQUEST[item]).")";
		$dbu->qry($sql);
		$_SESSION['msg']="data destinasi_biaya yang terpilih berhasil dihapus ....";
		$_SESSION['alt']="danger";
...
*/
		...
		$appx->mq_encode('p_desk,p_harga');
		$sql = "insert into ".$app['table']["destinasi_biaya"]." (id_user, id_bahasa, id_destinasi, harga, deskripsi, tgl_post) values
				('".$app[me][id]."','$p_bahasa','$p_id','$p_harga','".$p_desk."',now())";
		//echo $sql;exit;
		$dbu->qry($sql);
		
		
		$_SESSION['msg'] = "Biaya Destinasi Berhasil ditambahkan ";
		$_SESSION['alt'] = "info";
		header("location: ".$urlx->get_referer());
		exit;
	}
...
if ($act == "update"){
	...
	$formix->init();
	if ($step == "1"){
		$page = $dbu->get_record("destinasi_biaya", "id = '".$sub."'");
		$form = $page;
		$formix->populate($form);
...
*/
		...
		$appx->mq_encode('p_desk,p_harga');
		$sql = "update ".$app['table']["destinasi_biaya"]."
