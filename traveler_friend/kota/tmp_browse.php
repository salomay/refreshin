...
<?php if($_SESSION['msg']!=""){ ?>
  ...
  print_r($rshasil);
  ?>
  <h2>Daftar <span class="label label-primary">Kota</span></h2>
  <br/> 
...
<form method="post">
  <div class="table_filter">
    <select class="form-control" name="fcari" >
      <option value="a.nama">kota</option>
      <option value="b.nama">provinsi</option>
  ...
  <table class="cmstable">
    <thead>
      <tr> 
        ...
        <th>
          <div><span>meta deskripsi</span></div>
        </th>
      </tr>
    </thead>
    <tbody>
      ...
      ?>
        <tr <?php echo($nors % 2 == 1)?'class="odd"':'class="even"'; ?>>
          ...
          <td>
            <?php echo $rshasil["provinsi"]; ?>&nbsp;&nbsp;
            <a href="../provinsi/index.php?act=view&p_id=<?php echo $rshasil[id_provinsi]; ?>" title="view detail" >
          ...
          <td style="text-align:center">
            <?php 
            if ($rshasil[logo]){ 
              $filename = $app[data_path]."/kota/logo/".$rshasil[logo];
              //echo $filename;    
            if(file_exists($filename)){
            ?>
              <img src="<?php echo  $app[data_www];?>/kota/logo/<?php echo $rshasil[logo];?>" width="50" height="50" />
          <?php }else{ ?>
          ...
          <td style="text-align:center;">
            <a href="../kota_bahasa/index.php?act=browse&p_id=<?php echo $rshasil[id]; ?>" title="tambah detil" >
