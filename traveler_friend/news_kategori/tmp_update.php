...
<?php if($_SESSION['msg']!=""){ ?>
  ...
  //print_r($app[me]);
  ?>
  <h2>Merubah Data <span class="label label-primary">Kategori Berita</span></h2>
  <br/> 
...
<div>
	...
	<div>
		<?php 
		if ($form['icon']){ 
				$filename = $app['data_path']."/berita/icon/".$form['icon'];
			if (file_exists($filename)) {
			 ?>
			 	<img src="<?php echo  $app['data_www'];?>/berita/icon/<?php echo $form[icon];?>" width="50" height="50" /> <br/><br/> 
				<input type="checkbox" class="checkbox" name="p_thumb_del" value="1">check to remove <br /><br />
			 <?php
			} else {
			 ?>
			 	<img src="<?php echo  $app['data_www']?>/berita/icon/default.png" width="50" height="50" />
				<br><br>
			 <?php }
			} else { ?>
			<img src="<?php echo  $app['data_www']?>/berita/icon/default.png" width="50" height="50" />
...
<div>
	...
	<div>
		<?php 
		if ($form['nicon']){ 
				$filename = $app['data_path']."/berita/icon/".$form['nicon'];
			if (file_exists($filename)) {
			 ?>
			 	<img src="<?php echo  $app['data_www'];?>/berita/icon/<?php echo $form[nicon];?>" width="50" height="50" /> <br/><br/> 
				<input type="checkbox" class="checkbox" name="p_thumb_del" value="1">check to remove <br /><br />
			 <?php
			} else {
			 ?>
