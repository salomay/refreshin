...
<?php if($_SESSION['msg']!=""){ ?>
  ...
  //print_r($rshasil);
  ?>
  <h2>Daftar <span class="label label-primary">Kategori Berita</span></h2>
  <br/> 
      ...
      ?>
        <tr <?php echo($nors % 2 == 1)?'class="odd"':'class="even"'; ?>>
          ...
          <td style="text-align:center">
            <?php 
            if ($rshasil[icon]){ 
              $filename = $app[data_path]."/berita/icon/".$rshasil[icon];
              //echo $filename;    
            if(file_exists($filename)){
            ?>
              <img src="<?php echo  $app[data_www];?>/berita/icon/<?php echo $rshasil[icon];?>" width="50" height="50" />
          <?php }else{ ?>
