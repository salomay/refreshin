...
if ($act == "browse"){
	...
	$sql = "SELECT a.*, b.bahasa as bahasa FROM ".$app['table']["berita_kategori"]." a LEFT JOIN ".$app['table']["bahasa"]." b ON(a.id_bahasa=b.id) $where $sort $limit";
	//echo $sql;exit;
...
if ($act == "delete"){
	if (!empty($_REQUEST[item])){
		$sql = "DELETE FROM ".$app['table']["berita_kategori"]." WHERE `id` IN (".$admlib->item_delete($_REQUEST[item]).")";
		$dbu->qry($sql);
    ...
    if ($step ==2){
		...
		if ($p_nicon_size > 0){
			...
			try{
				...
				$imgx->thumbnail(55, 55);
				$imgx->save($app['data_path']."/berita_kategori/icon/nicon_".$id.".png");				
...
*/
		...
		$sql = "insert into ".$app['table']["berita_kategori"]." (kategori, id_bahasa, icon) values
				('$p_kategori', '$p_bahasa', '$data[icon]')";
		//echo $sql;exit;
		$dbu->qry($sql);
...
if ($act == "update"){
	...
	if ($step == "2"){
		...
		## check duplicate berita_kategori 
		$nama = $dbu->lookup("kategori","berita_kategori", "kategori ='".$p_kategori."' AND id_bahasa ='".$p_bahasa."' and id <>'".$p_id."'");
		if ($nama){
		...
		if ($_SESSION['msg']!=""){
			...
			//----icon
		$data[icon]= $dbu->lookup("icon","berita_kategori","id='".$p_id."'");
			if ($p_icon_size){
				$id = rand(1, 100).date("dmYHis");
				
				@unlink($app['data_path']."/berita/icon/$data[icon]");	
				$data['icon'] = "";
				//echo "masuk";exit;
				try{
					$src_img = $_FILES["p_icon"]['tmp_name'];
...
/*
		...
		$data[nicon]= $dbu->lookup("nicon","berita_kategori","id='".$p_id."'");
			if ($p_nicon_size){
				...
				@unlink($app['data_path']."/berita_kategori/icon/$data[nicon]");	
				$data['nicon'] = "";
				//echo "masuk";exit;
				try{
					$src_img = $_FILES["p_nicon"]['tmp_name'];
