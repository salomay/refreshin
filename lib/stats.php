...
/*******************************************************************************
* Filename : ijs_stats.php
* Description : ijs_stats library
*******************************************************************************/
...
function geoip_country_code_by_name($gi, $name) {
  $country_id = stats::geoip_country_id_by_name($gi,$name);
  //echo $GLOBALS['GEOIP_COUNTRY_CODES'][$country_id];exit;
  if ($country_id !== false) {
...
function geoip_country_id_by_addr($gi, $addr) {
  $ipnum = ip2long($addr);
  return stats::_geoip_seek_country($gi, $ipnum) - GEOIP_COUNTRY_BEGIN;
}

function geoip_country_code_by_addr($gi, $addr) {
  $country_id = stats::geoip_country_id_by_addr($gi,$addr);
  //print_r($GLOBALS['GEOIP_COUNTRY_CODES'])[$country_id];exit;
  if ($country_id !== false) {
    return $GLOBALS['GEOIP_COUNTRY_CODES'][$country_id];
  }
  return false;
...
/***************************************************************************
* Description : log user sorini_stats
 ***************************************************************************/
function log_stats($page_id, $act='', $p_id = '',$sub = '',$step = '', $cat='', $status='',$tab='')
{
	global $app, $HTTP_USER_AGENT, $REMOTE_ADDR,$GEOIP_COUNTRY_CODE_TO_NUMBER;
	...
	$browser_name = $current_browser->browser_name;
	$browser_detail = (isset($current_browser->Parent)) ? $current_browser->Parent : 'Unknown';
	$os = $current_browser->Platform;
	$mobile = ($current_browser->isMobileDevice != "")? $current_browser->isMobileDevice : 0 ;
	$app['isMobile'] = $mobile;
	$crawler = ($current_browser->Crawler != "")? $current_browser->Crawler : 0 ;
		
	$gi = stats::geoip_open("$app[path]/lib/GeoIP.dat",GEOIP_STANDARD);
	$negara = stats::geoip_country_name_by_addr($gi, $ip);     
	$negara = $negara ? $negara : "Local Network";
	$id_negara = stats::geoip_country_code_by_addr($gi, $ip);     
	$id_negara = $id_negara ? $id_negara : "loc";*/
	...
	$ip = $REMOTE_ADDR;
	$resolution = $_SESSION['resolusinya'];
	$refer = parse_url(url::get_referer()); 
	$url_referer = (!isset($_SERVER['HTTP_REFERER'])) ? 'Direct' : $_SERVER['HTTP_REFERER'];
	
	
	$id = rand(1, 100).date("dmYHis");
	$sql = "insert into ".$app['table']['statistic']." (id, page_id, referer_base, browser_detail, browser_name, mobile, crawler, ip, browser, os, tgl, resolution, country, country_id, referer) values('$id', '$page_id', '$referer_base', '$browser_detail', '$browser_name','$mobile','$crawler', '$ip', '$browser', '$os', '$tgl', '$resolution', '$negara', '$id_negara', '$url_referer')";
	db::qry($sql);*/
