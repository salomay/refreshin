...
function menu(){
	...
	$('.overlay-body-menu').height($(window).height());
	$(".burger-menu").click(function(e){
		if($(this).hasClass("is-active")){
			$(this).removeClass("is-active");
			$(".wrapmenu").fadeOut("fast");
			$(".overlay-body-menu").fadeOut("fast");
		}else{
			$(this).addClass("is-active");
			$(".wrapmenu").fadeIn();
			$(".overlay-body-menu").fadeIn();
		}
	});
	$(".overlay-body-menu").click(function(e){
...
function fly() {
	$(window).scroll(function(){
		...
		$('.head_nf_community').css({position:'fixed', top:'60px', width:"72%"});
		}else{
			$('.head_nf_community').css({top:'0','position':'relative', width:"100%"});
		}
	})
}
